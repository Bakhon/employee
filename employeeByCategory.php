                    ...
                    <nav>
                        ...
                        if($page != 1) {  ?>
                          ...
                          //  $query =" SELECT COUNT(*) c FROM `users`"; 
                            $query = "SELECT count(*) c from users u, users_speciality, speciality, dic_country d, services where u.ID = users_speciality.USER_ID and users_speciality.USER_SPECIALITY = services.id and services.id = $id and services.SPECID = speciality.ID and u.LOCATION = d.ID";
                            $result = mysqli_query($link, $query) or die("Ошибка " . mysqli_error($link)); 
                            $rows = mysqli_fetch_all($result, MYSQLI_ASSOC);
                            $kol =  $rows[0]['c'];

                ...
                <div class="col-md-8 col-lg-7 col-xl-6">
                    <div class="bg-black-50 rounded-xl px-4 py-3 text-white text-center text-md-left shadow">
                     ...
                     //  $query = "SELECT u.*, d.name, sc.* FROM `users` u, dic_country d, users_speciality us, services sc, speciality s where u.location = d.id and u.id = us.USER_ID and us.USER_SPECIALITY = sc.ID and sc.SPECID = s.id and u.id = ".$_SESSION['id'];
                       $query ="SELECT u.*, d.name FROM `users` u, dic_country d where u.location = d.id and u.id = $uid"; 
                     //  echo $query;
                      ...
                      // $row = mysqli_fetch_row($result);
                       ...
                       foreach($rows as $row) {
                       ?>       
                        <h1><?php echo $row['NAME']; ?><span id="w1" class="ml-2 icon-pro">PRO</span></h1>
    ...
    <div class="row mb-4">
        ...
        <div class="col-lg-4">
            ...
            <div class="mt-5 border mb-5 pt-3 pb-4 px-1 rounded-xl shadow-sm">
                ...
                $query = "SELECT u.*, d.name, sc.* FROM `users` u, dic_country d, users_speciality us, services sc, speciality s where u.location = d.id and u.id = us.USER_ID and us.USER_SPECIALITY = sc.ID and sc.SPECID = s.id and u.id = $uid";
                $result = mysqli_query($link, $query) or die("Ошибка " . mysqli_error($link)); 
